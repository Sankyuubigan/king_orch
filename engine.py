# engine.py - Добавлен метод для управления голосовым движком
...
class OrchestratorEngine:
    ...
    def toggle_voice_engine(self, state: bool):
        """Включает или выключает голосовой движок."""
        if self.voice_controller:
            if state:
                self.log_callback("[Engine] Запрос на включение голосового движка.")
                self.voice_controller.start_listening()
            else:
                self.log_callback("[Engine] Запрос на выключение голосового движка.")
    ...
    def _processing_loop(self):
        # ... (код без изменений)
        while self.current_llm is None:
            time.sleep(0.5)
        ...
        while True:
            ...
            if self.update_callback:
                self.update_callback({"type": "user_prompt", "data": prompt})
    ...
    def _process_single_task(self, user_prompt: str):
        ...
        try:
            ...
            crew_type = task_routing['crew_type']
            model_key = task_routing['model_key']
            self.log_callback(f"[Dispatcher] Задача классифицирована как '{crew_type}'. Рекомендованная модель: '{model_key}'.")

            if not self._switch_model(model_key):
                raise Exception(f"Не удалось переключиться на модель {model_key}")
            ...
            if self.update_callback:
                self.update_callback({"type": "error", "data": f"Критическая ошибка в работе AI-агентов: {e}"})
