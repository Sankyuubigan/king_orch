...
### Правила песочницы (СТРОГО)
- Путь: ТОЛЬКО `.agents_workspace/sandbox/` (относительно корня проекта)
- Sandbox поддерживает ТОЛЬКО TypeScript/JavaScript (.ts/.js)
- Python/другие ЯЗЫКИ НЕ ДОСТУПНЫ — Deno sandbox
- ЗАПРЕЩЕНО создавать файлы в `sandbox/` корня проекта
...
4. Твой ОТВЕТ — ТОЛЬКО JSON объект, без markdown, без пояснений, без заголовков.
   Если тест упал там, где ожидалось: {"bug_captured": true, "logs": "..."}
   Если тест не падает: {"bug_captured": false}
...
3. Твой ОТВЕТ — ТОЛЬКО JSON объект, без markdown, без пояснений, без заголовков.
   Если тест прошел успешно: {"pass": true}
   Если тест упал: {"pass": false, "reason": "..."}
