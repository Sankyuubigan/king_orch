// MCP-сервер YouTube (Deno): скачивание субтитров через yt-dlp, очистка от спонсорских сегментов,
// разбивка на части. Требует yt-dlp в bins (KING_ORCH_BINS_DIR) и права --allow-run.
...
createMcpServer({
    ...
    handlers: {
        prepare_video: (args: Record<string, unknown>) => prepareVideo(String(args.url || ''), Number(args.chunk_size) || 12000),
        get_chunk: (args: Record<string, unknown>) => getChunk(String(args.url || ''), Number(args.chunk_index))
    }
});

function getChunk(url: string, chunkIndex: number): string {
...
function prepareVideo(url: string, chunkSize: number): string {
    ...
    try {
        ...
        if (sponsorData && !sponsorData.includes("Not Found")) sponsors = JSON.parse(sponsorData).map((item: { segment: [number, number] }) => item.segment);
    } catch (e) {}
    ...
    let ytDlpCmd = 'yt-dlp';
    const cwd = Deno.cwd();
    const here = import.meta.dirname || cwd;
    const searchPaths = [path.join(cwd, 'bin', ytName), path.join(cwd, ytName), path.join(here, '..', '..', 'bin', ytName)];
    const binsDir = Deno.env.get("KING_ORCH_BINS_DIR");
    if (binsDir) searchPaths.push(path.join(binsDir, ytName));
    for (const p of searchPaths) { if (fs.existsSync(p)) { ytDlpCmd = `"${p}"`; break; } }

    let cookiesArg = "";
    for (const p of [path.join(cwd, 'cookies'), path.join(here, '..', '..', 'cookies')]) {
        if (fs.existsSync(p)) {
            const cFiles = fs.readdirSync(p);
            const txtFile = cFiles.find(f => f.endsWith('.txt'));
            if (txtFile) { cookiesArg = `--cookies "${path.join(p, txtFile)}"`; break; }
        }
    }
    ...
    for (let line of lines) {
        ...
        if (line && !line.includes('WEBVTT') && !line.includes('-->') && !line.match(/^\d+$/)) {
            let isSponsor = false;
            for (const [start, end] of sponsors) { if (currentStart >= start && currentStart <= end) { isSponsor = true; break; } }
            if (!isSponsor) {
    ...
    for (let line of cleanText.split('\n')) {
        if (currentChunk.length + line.length > chunkSize && currentChunk.length > 0) { chunks.push(currentChunk); currentChunk = ""; }
        currentChunk += line + '\n';
    }
