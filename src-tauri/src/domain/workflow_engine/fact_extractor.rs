...
pub(crate) fn build_default_prompt(facts: &[FactDef], phases: &[FactDef], output_fields: &[OutputFieldDef], user_message: &str, signals: &str, history: &str) -> String {
    ...
    prompt.push_str(
        "\n\n### Жёсткое правило трактовки критериев\n\
         Строго следуй написанным критериям каждого факта. Не делай собственных выводов, \
         противоречащих критерию, — критерии могут прямо требовать противоположного тому, что \
         кажется «общим смыслом». Оценивай каждый факт независимо, по его собственному критерию.",
    );
