...
pub(crate) fn build_default_prompt(facts: &[FactDef], phases: &[FactDef], output_fields: &[OutputFieldDef], user_message: &str, signals: &str) -> String {
    ...
    prompt.push_str(
        "\n\n### Жёсткое правило трактовки критериев\n\
         ...
         - Если критерий `scenario_established` требует FALSE, когда юзер отверг предложенные гроундером сценарии — ставь scenario_established=false, ДАЖЕ ЕСЛИ в истории сессии уже есть сообщение от агента grounder. Сообщение grounder лишь ПРЕДЛАГАЕТ варианты; сценарий считается установленным (grounded) только после явного подтверждения резонанса от самого юзера. Соматический симптом не может быть 6-м элементом невроза — ищи ментальный уровень события.\n\
