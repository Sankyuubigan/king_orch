...
pub(crate) fn log_agent_thought(
    ...
    if thinking_sec > 0.0 {
        log_cb(format!(
            "💭 Мысль {} [d={}] ({} {}) [⏱{:.1}с]: {}",
            agent.name, depth, action_type, target, thinking_sec, thought
        ));
    } else {
        log_cb(format!(
            "💭 Мысль {} [d={}] ({} {}): {}",
            agent.name, depth, action_type, target, thought
        ));
    }
}

pub(crate) fn valid_agent_ids(
    agents: &[AgentProfile],
    exclude_id: &str,
    exclude_mode: &str,
) -> Vec<String> {
