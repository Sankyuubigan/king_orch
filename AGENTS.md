...
### Фронтенд-агенты — терминальные узлы пайплайна (НЕ вешать исходящие рёбра)

**Агенты из папки `frontend/` — КОНЕЧНЫЕ (терминальные) узлы графа workflow.** Они выдают результат пользователю (сценарии, вопросы, сообщение) и завершают прогон. Обработка продолжается только тогда, когда пользователь ответит в чат и перезапустит обработку (пайплайн перезагружается с начала графа, подхватывая новый ответ из истории сессии).

- **ЗАПРЕЩЕНО подключать исходящие рёбра (`edges`) к фронтенд-агентам** — они завершают прогон по определению. Любое `→ frontend_agent → ...` в графе — архитектурная ошибка.
- Если после фронтенд-агента нужно продолжить логику — этого не требуется: юзер ответит, пайплайн перезапустится, и маршрутизация пойдёт дальше по графу на основе обновлённого состояния (сигналов/фактов), без явного ребра от фронтенд-агента.
- Это правило не относится к `backend/` агентам — те могут иметь рёбра и передавать данные дальше внутри одного прогона.
