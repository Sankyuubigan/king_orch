...
>
> ### 3. Документация проекта в `docs/`
> **В `docs/` лежит документация по проекту.** Перед любыми изменениями архитектуры, API или поведения — сначала прочитать соответствующие файлы оттуда. После изменений — обновить `docs/ARCHITECTURE.md` и/или `docs/RULES.md`.
>
> **История:** агент перепроектировал систему агентов, не прочитав `docs/ARCHITECTURE.md`, и создал несовместимую архитектуру.
>
> ### 4. Запрет на любые git операции без разрешения пользователя
...
- Обратная совместимость: сессии хранятся в новом формате (`type`/`author`). Автоматическая миграция старых сессий (`role`/`agent_name` → `type`/`author`) удалена в v26.5.75 — считается, что у всех пользователей актуальная версия.
...
- Если сборка падает с CUDA/nvcc — CUDA 12.9+ несовместима с llama-cpp-sys-2 0.1.146 (ошибка `__cudaRegisterEntry` macro). Временно отключить CUDA в Cargo.toml (`features = ["mtmd"]` вместо `["cuda", "mtmd"]`).
- Удаление `target/debug/build/llama-cpp-sys-2-*` — критическая ошибка (см. ПРИОРИТЕТ №1 в начале файла)

### Правила для тестов (важно!)
- **Таймаут на любые тесты — не более 2 минут (120 000ms) (`timeout: 120000` в tool call).** Если тест не укладывается — значит что-то не так, не жди дольше.
- **LLM-тесты (с реальной моделью) — только `#[ignore]`.** Запускать строго через `cargo test -- --ignored` по явной просьбе пользователя.
- **Перед любым `cargo test` — проверь `sccache --start-server`.**
