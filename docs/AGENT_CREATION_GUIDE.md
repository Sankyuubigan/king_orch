...
Если указать `single_report: true`, то перед сохранением нового отчёта движок **удаляет все прошлые сообщения этого же автора** (по полю `author = ID агента`). В сессии остаётся только один — самый свежий — отчёт агента.
...
- Рекомендуется для «шумных» воркеров, которые вызываются многократно (например: `soma_translator`, `validator`, `decomposer`).
