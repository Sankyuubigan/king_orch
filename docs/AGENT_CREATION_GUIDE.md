...
### Правила формулировки имён фактов и критериев

Факты (`id`) и их `criteria` в `facts.yaml` пишутся ПО ЕДИНОМУ СТАНДАРТУ (для всех команд, на будущее). Экстрактор фактов — слабое звено: им часто управляет не самая сильная LLM, поэтому критерии должны быть максимально простыми и однозначными.

1. **Имя факта — простое и понятное.** Короткий `snake_case` из общеупотребимых слов. Суть факта должна быть ясна из одного имени, без чтения критерия.
2. **Критерий — перцептивный (наблюдаемый).** Описывай то, что реально видно в тексте пользователя (конкретные слова, фразы, описанные ситуации), а не абстракции и домыслы.
3. **Критерий — явный и позитивный.** Формулируй через «факт ИСТИНА, когда…», а не через запреты («не путай», «не делай так»). Позитивная инструкция понятнее слабым моделям.
4. **Максимальная простота.** Один критерий = одно проверяемое условие. Без вложенной логики, без отсылок к «общему смыслу» и «контексту за кадром».
5. **Прозрачность.** По критерию однозначно понятно, почему факт `true`/`false`. Никакой скрытой интерпретации.
6. **Критерии не противоречивы.** Разные факты не должны требовать взаимоисключающих трактовок одного и того же текста. Если факты пересекаются — явно опиши их границы (что относится к одному, а что к другому).
7. **Ориентация на слабые LLM.** Пиши так, чтобы модель с минимальными способностями точно выполнила инструкцию: без двусмысленности, многоэтажных предложений и скрытых допущений.
8. **Значение факта очевидно из данных.** За значением факта (`true`/`false`) НЕ должно стоять ничего, чего нет во входных данных (сообщение пользователя + сигналы). Не требуй от модели внешних знаний или догадок.

**Пример (плохо → хорошо):**

```yaml
# ПЛОХО: абстрактно, негативно, двусмысленно
- id: has_weird_body_stuff
